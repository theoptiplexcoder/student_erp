          ...
          {mockApplications.map((app) => (
            ...
            >
              <td className="text-primary px-4 py-3 font-medium">
                <Link
                  href={`/tenant-admin/students/admissions/${app.id}`}
              ...
              <td className="px-4 py-3">
                <span
                  className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${app.status === 'Submitted' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : ''} ${app.status === 'Under Review' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : ''} ${app.status === 'Offer Sent' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : ''} `}
                >
                  {app.status}
                </span>
              </td>
