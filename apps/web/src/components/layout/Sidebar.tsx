...
const sidebarGroups = [
  {
    title: 'Dashboard',
    items: [{ name: 'Overview', icon: LayoutDashboard, href: '/tenant-admin' }],
  },
  {
        ...
        {sidebarGroups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="flex flex-col gap-1">
              ...
              <div
                className={`flex flex-col gap-1 overflow-hidden transition-all duration-200 ${!isExpanded && !collapsed ? 'max-h-0 opacity-0' : 'max-h-[1000px] opacity-100'}`}
              >
