...
export const AdminApi = {
  ...
  timetable: {
    ...
    update: (id: string, data: any) =>
      adminApiClient.patch(`/timetable/${id}`, data).then((res) => res.data),
    delete: (id: string) => adminApiClient.delete(`/timetable/${id}`).then((res) => res.data),
    generate: (data: any) =>
      adminApiClient.post('/timetable/generate', data).then((res) => res.data),
    validate: (termId: string) =>
      adminApiClient.post('/timetable/validate', { termId }).then((res) => res.data),
    publish: (termId: string) =>
      adminApiClient.post('/timetable/publish', { termId }).then((res) => res.data),
    duplicate: (data: { fromTermId: string; toTermId: string }) =>
      adminApiClient.post('/timetable/duplicate', data).then((res) => res.data),
    export: (params: { termId: string; format: 'csv' | 'json' }) =>
      adminApiClient.get('/timetable/export', { params }).then((res) => res.data),
    import: (data: FormData) =>
      adminApiClient.post('/timetable/import', data).then((res) => res.data),
    move: (id: string, data: any) =>
      adminApiClient.post(`/timetable/${id}/move`, data).then((res) => res.data),
    reassignFaculty: (data: any) =>
      adminApiClient.post('/timetable/reassign-faculty', data).then((res) => res.data),
    bulkUpdate: (data: any) =>
      adminApiClient.post('/timetable/bulk-update', data).then((res) => res.data),
    bulkDelete: (entryIds: string[]) =>
      adminApiClient.post('/timetable/bulk-delete', { entryIds }).then((res) => res.data),
    swapSlots: (entryIdA: string, entryIdB: string) =>
      adminApiClient.post('/timetable/swap-slots', { entryIdA, entryIdB }).then((res) => res.data),
    conflicts: (termId: string) =>
      adminApiClient.get('/timetable/conflicts', { params: { termId } }).then((res) => res.data),
  },
  facultySections: {
    create: (data: {
      facultyId: string;
    ...
    update: (id: string, data: { role?: string; isPrimary?: boolean }) =>
