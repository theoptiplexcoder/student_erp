...
export const StudentApi = {
  ...
  getCourseDetails: (courseId: string) => studentApiClient.get(`/academic/courses/${courseId}`).then(res => res.data),
  getAttendanceSummary: () => studentApiClient.get('/attendance').then(res => res.data),
  getCourseAttendance: (courseId: string) => studentApiClient.get(`/attendance/${courseId}`).then(res => res.data),
