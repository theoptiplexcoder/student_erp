...
Before the individual journeys, three structural facts from `project_structure.md` shape *everything* below:
...
*Persona ref: `personas.md` §20.1 · Primary app: `app-placement` (external-facing view)*
...
2. Notification Service sends an invite (email/SMS per institution config) with a scoped account-setup link — scoped meaning it grants access to *this* hiring cycle only, not a standing account across all cycles.
...
*Persona ref: `personas.md` §20.2 · Primary app: `portal-vendor`*
...
### 2.3 Exception Flows
- **Invoice discrepancy** — Flagged back to the originating department's staff (Facilities/IT/Finance) for resolution *before* Finance processes payment; the invoice sits in a disputed state, not silently rejected.
...
- **Distinction worth noting:** unlike the Payment Gateway machine actor (§21.2 of `user_journey.md`), which handles *online* student/guardian fee payments, vendor payments are accounts-payable and may run through offline banking rails (bank transfer, cheque) rather than the same payment gateway integration — the event names are shared conceptually but the settlement mechanism differs.
...
*Persona ref: `personas.md` §20.3 · Primary app: `portal-auditor`*
...
*Persona ref: `personas.md` §20.6 · Primary app: `portal-corporate-client`*
...
|---|---|---|---|---|
| Provisioned by | Placement Staff | Facilities / Finance / IT Staff | Institution Administrator / Platform Security | Institution Administrator / Finance Staff *(inferred)* |
